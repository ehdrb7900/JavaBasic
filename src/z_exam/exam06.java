...
//		답 : return Math.sqrt((x1 - x)*(x1 - x) + (y1 - y)*(y1 - y));
//			
		
//		[6-7] 문제6-6에서 작성한 클래스 메서드 getDistance()를 MyPoint클래스의 인스턴스 메서드로 정의하시오.
//			class MyPoint { 
//			int x;
//			int y;
//			
...
//		답 : double getDistance(int x, int y){
...
//		답 : 	static은 객체의 생성없이 변수나 메소드를 사용하기위해, 
//				그리고 객체들간에 변수의 값을 공유하기 위해 사용된다.
//				위 예제의 경우 모든 병사의 공격력과 방어력이 같아야 하므로
//				weapon과 armor 변수에 static이 붙어야 한다.
//				또한 weaponUp()과 armorUp()은 static이 붙은 변수를 
//				사용하는 메소드이기 때문에 static이 붙어야한다.
//			
...
//		답 : b. 생성자는 인스턴스 변수를 초기화하기 위한 것이다.
//			e. 생성자의 파라미터 개수나 자료형이 다를 경우 오버로딩이 가능하다.
//		
...
//		답 : c. 리턴타입이 같더라도 매개변수의 개수가 다르다면 성립 조건에 부합한다.
//			d. 매개변수의 이름과는 관련이 없다.
//		
...
//		답 : c. 초기화 순서는 (명시적 초기화 = 초기화 블럭) > 생성자이다.
//			e. 클래스 변수의 경우 먼저 메모리에 올라가기 때문에 클래스변수가 먼저 초기화된다.
//		
...
//		답 : 라인 A - 클래스 변수가 먼저 메모리에 올라가고 인스턴스 변수가 나중에 올라가기 때문에
//					값을 대입할 수 없다.
//			라인 B - 클래스 메소드에서는 인스턴스 변수를 사용하지 못하게 되어있다. 
//			라인 D - 클래스 메소드에서는 인스턴스 메소드를 사용하지 못하게 되어있다.
//		
//		[6-19] 다음 코드의 실행 결과를 예측하여 적으시오. ------- > 잘 모르겠음
//		
//			class Exercise6_19 { 
...
//					처리한 배열을 반환한다.
//			반환타입 : int[] 매개변수 : int[] arr - 정수값이 담긴 배열
//			
//			class Exercise6_20
//			{
//			/* (1) shuffle메서드를 작성하시오. */
...
//		답 : public static int[] shuffle(int[] arr){
...
//			1234o는 숫자입니까? false
//					
//		답 : public static boolean isNumber(String str){
...
//		답 : public static int max(int[] arr){
...
//		답 : public static int abs(int value){
//				if(value > 0){
//					return value;
//				} else {
//					return -value;
