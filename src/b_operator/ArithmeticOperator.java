...
public class ArithmeticOperator {
	...
	public static void main(String[] args) {
		...
		// 피연산자의 타입이 서로 다를 경우
		// 표현범위가 큰 쪽으로 형변환 후 연산이 수행된다.
		// 따라서 연산의 결과도 표현범위가 큰 쪽의 타입이 된다.
		
		byte b = 10;
		short c = 20;
		
		short d = (short) (c - b); // 자동 형변환은 주로 4바이트가 기준
		...
		float f1 = 10 / 4;
		System.out.println(f1); // 2.0 출력
		float f2 = 10 / 4f;
		System.out.println(f2); // 2.5 출력
		int f3 = 10 % 4; // 10을 4로 나눈 나머지
		System.out.println(f3); // 2 출력
		
		// 정수는 0으로 나눌 수 없다.
		// int g1 = 10 / 0; <- 런타임 에러 발생 (컴파일 에러 X)
		float g2 = 10.0f / 0;
		System.out.println(g2); // Infinity 출력
		float g3 = 0 / 0f;
		System.out.println(g3); // NaN : Not a Number 출력
		
		char h1 = 'A';
		char h2 = (char) (h1 + 1);
		System.out.println(h2); // B 출력
		
		int h3 = 'D' - 'A'; // 68 - 65
		System.out.println(h3); // 3 출력
		...
		System.out.println(Math.round(round * 10) / (double)10);
		System.out.println(Math.round(round * 10) / 10.0);
		System.out.println((int)((round) * 10 + 0.5) / 10.0); // 소수점 둘쨰자리에서 반올림
		
		// 연산을 할 때는 연산의 범위와 타입을 고려하자
		
		// 문제 1. 다음을 한 줄씩 계산해서 최종 결과값을 출력해주세요.
		// 1. 123456 + 654321
		// 2. 1번의 결과값 * 123456
		// 3. 2번의 결과값 / 123456
		// 4. 3번의 결과값 - 654321
		...
		long n = 123456 + 654321;
		System.out.println("1번 : " + n);
		n *= 123456;
		System.out.println("2번 : " + n);
		n /= 123456;
		System.out.println("3번 : " + n);
		n -= 654321;
		System.out.println("4번 : " + n);
		n %= 123456;
		System.out.println("5번 : " + n);
		
		// 문제 2. 3개의 int형 변수를 선언 및 초기화 후
		// 합계와 평균을 구해주세요. (평균은 소수점 둘째자리에서 반올림)
		
		int alpha = 123;
		int beta = 223;
		int gamma = 333;
		System.out.println("합계 : " + (alpha + beta + gamma));
		System.out.println("평균 : " + ((int)((alpha + beta + gamma) / 3.0 * 10 + 0.5) / 10.0));
		}
